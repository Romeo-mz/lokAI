...
func ShowHereticSuggestion() {
	...
	fmt.Println(SubtitleStyle.Render("🖼  Uncensored Image & Video Generation"))
	fmt.Println()
	fmt.Println("   For uncensored image/video generation, use Stable Diffusion with Uncensored checkpoints:")
	fmt.Println()
	...
	fmt.Println("   " + ValueStyle.Render("CivitAI") + "        — " + MutedStyle.Render("https://civitai.com (browse Uncensored checkpoints & LoRAs)"))
	fmt.Println()
